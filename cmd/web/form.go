...
func MinCharPtr(value *string, n int) bool {
	return utf8.RuneCountInString(*value) >= n
}
...
func MaxCharPtr(field *string, n int) bool {
	clean := strings.TrimSpace(*field)
	return utf8.RuneCountInString(clean) <= n
}
...
func (fs *FormSnippet) CheckForm() {
	...
	fs.CheckField(MaxCharPtr(&fs.Title, 150), "ErrorTitle", "Title too long")
	fs.CheckField(MaxCharPtr(&fs.Content, 1500), "ErrorContent", "Content too long")
...
func (f *userSignupForm) Resolve(r *http.Request) error {
	err := r.ParseForm()
	if err != nil {
		return err
	}
...
func (fs *userSignupForm) CheckForm() {
	...
	fs.CheckField(MinChar(fs.Name, 4), "ErrorName", "Name too short, 4 characters minimum")
